...
from basil_env import BasilEnv
...
class Agent:
    def __init__(self):
        ...
        self.Q = np.zeros((self.bounds[0]['upper'] - self.bounds[0]['lower'] + 1,) + (self.bounds[1]['upper'] - self.bounds[1]['lower'] + 1,) + (4,))
        self.epsilon = 1
    ...
    def decide_action(self, state:tuple) -> list:
        """
        Método usado para decidir a ação
        Inputs:
            state -> tupla que contem o estado atual do ambiente
        Outputs:
            action -> inteiro que determina a ação do agente
        """
    ...
    def update_q(self, current_state:tuple, action:list, reward:float, next_state:tuple):
        """
        Método para atualizar o valor de Q baseado na equação de Bellman
        Inputs:
            current_state -> tupla que contem o estado atual do ambiente
            action -> inteiro que determina a ação do agente
            reward -> float que contem o valor escalar da recompensa
            next_state -> tupla que contem o estado futuro do ambiente
        """
        #print(f"Current_state = {next_state}")
        #print(f"Next_state = {self.Q[next_state]}")
    ...
    def load_model(self, path) -> pickle:
        ...
        Inputs:
            path -> caminho que contém o modelo
        Outputs:
            modelo carregado
        """
    ...
    def train(self):
        """
        Metodo para treinar o ambiente
        """
